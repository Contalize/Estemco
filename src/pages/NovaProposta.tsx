...
export const NovaProposta: React.FC<NovaPropostaProps> = ({ onNavigate, editPropostaId }) => {
    ...
    const handleNext = () => {
        ...
        if (step === 2 && (!data.clienteId || !data.enderecoObra.cidade)) {
            setToast({ message: 'Preencha o cliente e a cidade da obra.', type: 'error' });
            return;
        }
        setStep(s => s + 1);
    };

    const handleSave = async (status: 'rascunho' | 'aprovada') => {
        ...
        try {
            ...
            if (data.tipo === 'HCM') {
                ...
                calc = calcularPropostaHCM(data.itens, data.mobilizacao);
                ...
                calc = calcularPropostaESC(data.itens, data.mobilizacao, data.modalidadeESC, data.precoFechadoESC, data.metrosDiariosESC, data.precoExcedenteESC);
