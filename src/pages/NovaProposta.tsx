...
import { montarTituloProposta } from '../lib/proposta-utils';
...
export const NovaProposta: React.FC<NovaPropostaProps> = ({ onNavigate, editPropostaId }) => {
    ...
    const handleNext = () => {
        ...
        if (step === 2 && (!data.clienteId || !data.enderecoObra.cidade || !data.enderecoObra.estado)) {
            toast.warning('Preencha o cliente, a cidade e o estado (UF) da obra.');
            return;
        }
        setStep(s => s + 1);
    };
    ...
    const handleSave = async (status: 'RASCUNHO' | 'ACEITA'): Promise<string | void> => {
        ...
        try {
            ...
            if (editPropostaId) {
                ...
                const titulo = montarTituloProposta({
                    numero: payload.numero || 'NOVA', // numero is generated by Firestore function usually but if we have it use it
                    tipo: payload.tipo,
                    cliente: payload.clienteNome,
                    cidade: payload.enderecoObra.cidade,
                    uf: payload.enderecoObra.estado,
                    data: new Date()
                });
