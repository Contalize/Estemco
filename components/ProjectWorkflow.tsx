    ...
    try {
      ...
      if (user?.uid && profile?.tenantId) {
        await logAudit(
          ...
          'STATUS',
          'obras',
          `Status da obra alterado para ${targetStatus}`,
          profile.tenantId
        );
      }
                ...
                filteredObras.map(o => (
                  <tr key={o.id} className="hover:bg-slate-50 cursor-pointer group">
                    ...
                    <td className="px-6 py-4 text-right">
                      <Button variant="action" className="opacity-0 group-hover:opacity-100 transition-opacity" title="Ver Detalhes" onClick={() => setSelectedObraId(o.id)}>
                        <FileText size={16} />
                      </Button>
                    </td>
                  </tr>
  ...
  const handleAdd = async () => {
    ...
    try {
      await addDoc(collection(db, 'anotacoes_obra'), {
        ...
        texto,
        autorNome: profile?.nome || 'Corpo Técnico',
        createdAt: serverTimestamp()
      });
      setTexto('');
    } catch (e) { }
