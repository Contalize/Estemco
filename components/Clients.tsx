...
import { Card, Button, Input, Skeleton, DropdownMenu, DropdownMenuTrigger, DropdownMenuContent, DropdownMenuItem } from './ui';
            ...
            <tbody className="divide-y divide-slate-200">
              {isLoading ? (
                ...
                paginatedClientes.map((cliente) => (
                  <tr key={cliente.id} className="bg-white hover:bg-slate-50 transition-colors cursor-pointer group">
                    ...
                    <td className="px-6 py-4">
                      <span className={`inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium ${cliente.status === 'Ativo' ? 'bg-emerald-100 text-emerald-800' : 'bg-slate-100 text-slate-800'
                        }`}>
