...
export const NovaProposta: React.FC<NovaPropostaProps> = ({ onNavigate, editPropostaId: externalEditId, autoOpenModal, onModalHandled }) => {
  ...
  const handleGeneratePDF = async (prop: PropostaData) => {
    if (!profile?.tenantId || !prop.id) return;
    setIsGeneratingPdf(prop.id);
    try {
      // Busca dados do cliente
      const clienteSnap = await getDoc(
        doc(db, 'clientes', prop.clienteId)
      );
      // Busca dados da empresa
      const empresaSnap = await getDoc(
        doc(db, 'empresas', profile.tenantId)
      );
      ...
      link.href = url;
      const clientNameSafe = prop.cliente.replace(/\s/g, '_');
      const propIdSafe = prop.id ? prop.id.substring(0, 8) : 'NOVO';
      link.download = `Proposta_${propIdSafe}_${clientNameSafe}.pdf`;
      link.click();
  ...
  const handleAprovarEGerarObra = async (prop: PropostaData) => {
    ...
    try {
      ...
      await logAudit(
        ...
        'APROVAÇÃO',
        'orcamento',
        `Proposta de ${prop.cliente} aprovada. Nova Obra gerada.`,
        profile.tenantId
      );
            ...
            <tbody className="divide-y divide-slate-200">
              {isLoading ? (
                ...
                filteredProposals.map((prop) => {
                  ...
                  return (
                    <tr key={prop.id} className="bg-white hover:bg-slate-50 transition-colors cursor-pointer group">
                        ...
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            {(() => {
                              ...
                              if (['Rascunho', 'Pendente'].includes(prop.status)) {
                                acoes.push({ label: 'Editar', icon: Edit2, acao: () => handleOpenModal(prop) });
                                acoes.push({
                                  label: 'Marcar como Enviada', icon: Send,
                                  acao: () => handleMarcarEnviada(prop.id)
                                });
                                acoes.push({
                                  label: 'Recusar', icon: X,
                                  acao: () => handleOpenRefuseModal(prop.id), className: 'text-red-600 hover:text-red-700 hover:bg-red-50'
                                });
                              }

                              if (prop.status === 'Enviada') {
                                acoes.push({
                                  label: 'Aprovar e Gerar Obra', icon: CheckCircle2,
                                  acao: () => handleAprovarEGerarObra(prop), className: 'text-emerald-600 hover:text-emerald-700 hover:bg-emerald-50'
                                });
                                acoes.push({
                                  label: 'Voltar para Pendente', icon: ArrowLeft,
                                  acao: () => handleReverterPendente(prop.id)
                                });
                                acoes.push({
                                  label: 'Registrar Recusa', icon: X,
                                  acao: () => handleOpenRefuseModal(prop.id), className: 'text-red-600 hover:text-red-700 hover:bg-red-50'
                                });
                              }

                              if (['Recusada', 'Expirada'].includes(prop.status)) {
                                acoes.push({
                                  label: 'Duplicar como Nova', icon: Copy,
                                  acao: () => handleDuplicar(prop)
                                });
                              }

                              if (prop.status === 'Aprovada') {
                                acoes.push({
                                  label: 'Ver Obra Gerada', icon: Eye,
                                  acao: () => handleVerObraGerada(), className: 'text-slate-700'
                                });
                              }

                              // Sempre disponível
                              acoes.push({
                                label: 'Gerar PDF', icon: isGeneratingPdf === prop.id ? Loader2 : Download,
                                acao: () => handleGeneratePDF(prop),
                                disabled: isGeneratingPdf === prop.id,
                                isPdf: true
                              });

                              return acoes.map((a, i) => (
                                <DropdownMenuItem key={i} onClick={a.acao} disabled={a.disabled} className={`flex items-center gap-2 ${a.className || ''}`}>
                                  {a.isPdf && a.disabled ? <a.icon size={14} className="animate-spin" /> : <a.icon size={14} />}
                                  {a.isPdf && a.disabled ? 'Gerando...' : a.label}
                                </DropdownMenuItem>
                              ));
                            })()}
                          </DropdownMenuContent>
