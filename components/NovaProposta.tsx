...
export const NovaProposta: React.FC<NovaPropostaProps> = ({ onNavigate }) => {
  ...
  // Filters & Refusal State
  const [statusFilter, setStatusFilter] = useState<string>('Todos');
  const [periodFilter, setPeriodFilter] = useState<string>('all');
  const [isRefuseModalOpen, setIsRefuseModalOpen] = useState(false);
  const [refusingProposalId, setRefusingProposalId] = useState<string | null>(null);
  const [refuseReason, setRefuseReason] = useState<string>('Preço');
  const [refuseObservation, setRefuseObservation] = useState<string>('');
  ...
  const handleGeneratePDF = async (proposal: PropostaData) => {
    if (!profile?.tenantId || !proposal.clienteId) {
      alert('Dados incompletos para gerar o PDF.');
      return;
    }

    setIsGeneratingPdf(proposal.id || null);

    try {
      // Fetch Company Data
      const companyDocRef = doc(db, 'empresas', profile.tenantId);
      const companyDocSnap = await getDoc(companyDocRef);
      const companyData = companyDocSnap.exists() ? companyDocSnap.data() : {} as any;

      // Fetch Client Data
      const clientDocRef = doc(db, 'clientes', proposal.clienteId);
      const clientDocSnap = await getDoc(clientDocRef);
      const clientData = clientDocSnap.exists() ? clientDocSnap.data() : {} as any;
      ...
      link.href = url;
      link.download = `Proposta-${proposal.id?.substring(0, 8).toUpperCase()}-${proposal.cliente.replace(/\s+/g, '_')}.pdf`;
  ...
  const carregarPropostas = async () => {
    ...
    try {
      ...
      const processedData = await Promise.all(data.map(async (p) => {
        const isPending = p.status === 'Pendente' || p.status === 'Rascunho' || p.status === 'Enviada' || !p.status;
        if (isPending && p.dataEmissao && p.validadeDias) {
          const emissao = new Date(p.dataEmissao);
          ...
          if (hoje > validade) {
            if (p.id) {
              await propostasService.atualizarProposta(p.id, {
                status: 'Expirada',
                expiradaEm: new Date().toISOString()
              });
              return { ...p, status: 'Expirada', expiradaEm: new Date().toISOString() };
            }
          }
        }
        return p;
  ...
  const handleMarcarEnviada = async (id: string | undefined) => {
    if (!id) return;
    try {
      await propostasService.atualizarProposta(id, { status: 'Enviada', dataEnvio: new Date().toISOString() });
      setToastMessage('Proposta marcada como Enviada!');
      setShowToast(true); setTimeout(() => setShowToast(false), 3000);
      carregarPropostas();
    } catch (e) { alert('Erro ao marcar enviada'); }
  };

  const handleReverterPendente = async (id: string | undefined) => {
    if (!id) return;
    try {
      await propostasService.atualizarProposta(id, { status: 'Pendente' });
      setToastMessage('Proposta retornou para Pendente!');
      setShowToast(true); setTimeout(() => setShowToast(false), 3000);
      carregarPropostas();
    } catch (e) { alert('Erro ao reverter status'); }
  };

  const handleDuplicar = async (prop: PropostaData) => {
    if (!profile?.tenantId) return;
    try {
      const { id, createdAt, updatedAt, dataAprovacao, dataRecusa, dataEnvio, expiradaEm, status, motivoRecusa, ...rest } = prop as any;
      await propostasService.criarProposta({
        ...rest,
        status: 'Rascunho',
        dataEmissao: new Date().toISOString()
      });
      setToastMessage('Proposta duplicada como Rascunho!');
      setShowToast(true); setTimeout(() => setShowToast(false), 3000);
      carregarPropostas();
    } catch (e) { alert('Erro ao duplicar proposta'); }
  };

  const handleOpenRefuseModal = (id: string | undefined) => {
    if (!id) return;
    setRefusingProposalId(id);
    setRefuseReason('Preço');
    setRefuseObservation('');
    setIsRefuseModalOpen(true);
  };

  const handleConfirmRefuse = async () => {
    if (!refusingProposalId) return;
    try {
      await propostasService.atualizarProposta(refusingProposalId, {
        status: 'Recusada',
        motivoRecusa: refuseReason === 'Outro' ? refuseObservation : refuseReason,
        dataRecusa: new Date().toISOString()
      });
      setIsRefuseModalOpen(false);
      setRefusingProposalId(null);
      setToastMessage('Proposta marcada como Recusada!');
      setShowToast(true); setTimeout(() => setShowToast(false), 3000);
      carregarPropostas();
    } catch (e) {
  ...
  const handleAprovarEGerarObra = async (prop: PropostaData) => {
    ...
    try {
      ...
      // ── PASSO 1: Atualizar status da Proposta ──────────────────
      const propostaRef = doc(db, 'propostas', prop.id);
      batch.update(propostaRef, {
        status: 'Aprovada',
        dataAprovacao: new Date().toISOString()
      });

      // ── PASSO 2: Criar documento em 'obras' ───────────────────
      const obraRef = doc(collection(db, 'obras'));

      // Calcular endereço da obra
      let enderecoDaObra = '';
      if (prop.mesmoEnderecoCliente) {
        // Busca o endereço do cliente fora do batch (read before write)
        const clienteSnap = await getDoc(doc(db, 'clientes', prop.clienteId));
        enderecoDaObra = clienteSnap.exists()
          ? (clienteSnap.data().enderecoFaturamento || 'Ver cadastro do cliente')
          : 'Ver cadastro do cliente';
      } else {
        const parts = [
          prop.obraEndereco,
          prop.obraNumero ? `nº ${prop.obraNumero}` : null,
          prop.obraBairro,
          prop.obraCidade && prop.obraUf ? `${prop.obraCidade}/${prop.obraUf}` : null
        ].filter(Boolean);
        enderecoDaObra = parts.join(', ');
      }

      // Calcular total de metros contratados
      const totalMetrosContratados = (prop.servicos || []).reduce(
        (acc: number, s: any) => acc + ((s.quantidade || 0) * (s.metragemPrevista || 0)),
        0
      );

      // Calcular dataPrevistaFim
      const dataPrevistaFim = new Date();
      dataPrevistaFim.setDate(dataPrevistaFim.getDate() + (prop.prazoExecucao || 30));
      ...
      batch.set(obraRef, {
        ...
        dataPrevistaFim: Timestamp.fromDate(dataPrevistaFim),
        totalMetrosContratados,
        totalMetrosExecutados: 0,
        ultimoBoletim: null,
      ...
      // ── PASSO 3: Criar 'dre_obras' com ID = obraId ────────────
      const dreRef = doc(db, 'dre_obras', obraRef.id);
      batch.set(dreRef, {
        tenantId: profile.tenantId,
        obraId: obraRef.id,
        propostaId: prop.id,
        clienteNome: prop.cliente,
        receitaContratada: prop.valorTotal || 0,
        receitaMedidaAcumulada: 0,
        totalMetrosExecutados: 0,
        totalDieselGasto: 0,
        totalHorasParadas: 0,
        totalHorasProducao: 0,
        totalCustoHorasParadas: 0,
        totalCustoMaoDeObra: 0,
        totalCustoConcreto: 0,
        totalBoletins: 0,
        overbreakMedio: 0,
        custoMobilizacao: prop.mobilizacao || 0,
        custoART: prop.solicitaART ? (prop.valorART || 0) : 0,
        updatedAt: Timestamp.now()
      } as DREObra);
      ...
      setToastMessage('✅ Proposta aprovada! Obra e DRE gerados com sucesso.');
      setShowToast(true);
      setTimeout(() => setShowToast(false), 4000);

      await carregarPropostas();

      // ── PASSO 4: Navegar para Obras ───────────────────────────
      if (onNavigate) onNavigate(Tab.WORKFLOW);

    } catch (error) {
      console.error('Erro ao aprovar proposta (batch):', error);
      alert('Erro ao aprovar proposta. Verifique sua conexão e tente novamente.');
    }
  };
  ...
  const getStatusBadge = (status: string) => {
    switch (status) {
      ...
      case 'Enviada': return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-blue-100 text-blue-800">Enviada</span>;
      ...
      case 'Rascunho': return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-slate-100 text-slate-800">Rascunho</span>;
      ...
      case 'Expirada': return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-orange-100 text-orange-800">Expirada</span>;
      default: return <span className="inline-flex items-center px-2.5 py-0.5 rounded-full text-xs font-medium bg-amber-100 text-amber-800">Pendente</span>;
    }
  };
  ...
  const currentMonthProposals = proposals.filter((p) => {
    const pDate = new Date(p.dataEmissao);
    return pDate.getMonth() === currentMonth && pDate.getFullYear() === currentYear;
  });

  const qtyGeradasMes = currentMonthProposals.length;

  const approvedMonth = currentMonthProposals.filter(p => p.status === 'Aprovada');
  const refusedMonth = currentMonthProposals.filter(p => p.status === 'Recusada');
  const totalApprovedRefused = approvedMonth.length + refusedMonth.length;

  const approvalRate = totalApprovedRefused > 0 ? ((approvedMonth.length / totalApprovedRefused) * 100).toFixed(1) : '0.0';
  const valorTotalAprovadoMes = approvedMonth.reduce((acc, curr) => acc + (curr.valorTotal || 0), 0);
  const ticketMedio = approvedMonth.length > 0 ? valorTotalAprovadoMes / approvedMonth.length : 0;
  ...
  const filteredProposals = proposals.filter(p => {
    ...
    if (statusFilter !== 'Todos') {
      const pStatus = p.status || 'Pendente';
    ...
    if (periodFilter !== 'all') {
      const pDate = new Date(p.dataEmissao);
      const [fMonth, fYear] = periodFilter.split('-');
      if (fMonth && fYear) {
        matchesPeriod = pDate.getMonth() === parseInt(fMonth) && pDate.getFullYear() === parseInt(fYear);
      }
    }

    let matchesSearch = true;
    if (searchQuery) {
      matchesSearch = (p.cliente || '').toLowerCase().includes(searchQuery.toLowerCase()) ||
    ...
    <div className="p-6 max-w-7xl mx-auto space-y-6">
      ...
      <div className="flex flex-col sm:flex-row justify-between items-start sm:items-center gap-4">
        <div>
          ...
          <p className="text-sm text-slate-500 mt-1">Gerencie seus orçamentos, contratos e acompanhe envios.</p>
        </div>
        <Button onClick={() => handleOpenModal()} className="gap-2 bg-slate-900 hover:bg-slate-800 text-white">
      ...
      <div className="grid grid-cols-1 sm:grid-cols-2 lg:grid-cols-4 gap-4">
        <Card className="p-4 bg-white border-slate-200">
          <p className="text-sm font-medium text-slate-500">Propostas Mês Atual</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-slate-900">{qtyGeradasMes}</span>
            <span className="text-sm text-slate-500">geradas</span>
          </div>
        </Card>
        <Card className="p-4 bg-white border-slate-200">
          <p className="text-sm font-medium text-slate-500">Taxa de Aprovação</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-slate-900">{approvalRate}%</span>
          </div>
        </Card>
        <Card className="p-4 bg-white border-slate-200">
          <p className="text-sm font-medium text-slate-500">Aprovado no Mês</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-emerald-600">{formatCurrency(valorTotalAprovadoMes)}</span>
          </div>
        </Card>
        <Card className="p-4 bg-white border-slate-200">
          <p className="text-sm font-medium text-slate-500">Ticket Médio (Aprovadas)</p>
          <div className="mt-2 flex items-baseline gap-2">
            <span className="text-2xl font-bold text-slate-900">{formatCurrency(ticketMedio)}</span>
          </div>
        </Card>
      </div>
        ...
        <div className="p-4 border-b border-slate-200 bg-white space-y-4">
          <div className="flex flex-col sm:flex-row gap-4 justify-between items-start sm:items-center">
            ...
            <div className="flex flex-wrap gap-2">
              {['Todos', 'Rascunho', 'Pendente', 'Enviada', 'Aprovada', 'Recusada', 'Expirada'].map(s => (
                <button
                  key={s}
          ...
          <div className="flex flex-col sm:flex-row gap-4 items-center">
            ...
            <Select value={periodFilter} onChange={(e) => setPeriodFilter(e.target.value)} className="w-full sm:w-48 bg-white">
              <option value="all">Todo o período</option>
              {Array.from({ length: 12 }).map((_, i) => {
                const d = new Date();
                d.setMonth(d.getMonth() - i);
                const val = `${d.getMonth()}-${d.getFullYear()}`;
            ...
            <tbody className="divide-y divide-slate-200">
              {isLoading ? (
                ...
                <tr>
                  <td colSpan={6} className="px-6 py-12 text-center">
                    <div className="flex flex-col items-center justify-center text-slate-500">
                      ...
                      <h3 className="text-lg font-medium text-slate-900 mb-1">Nenhuma proposta encontrada</h3>
                      <p className="text-sm mb-4">Ajuste os filtros de busca ou crie uma nova proposta.</p>
                ...
                filteredProposals.map((prop) => {
                  const propStatus = prop.status || 'Pendente';
                  return (
                      ...
                      <td className="px-6 py-4 text-right">
                        <DropdownMenu>
                          ...
                          <DropdownMenuContent align="end">
                            {/* PENDENTE / RASCUNHO */}
                            {(propStatus === 'Pendente' || propStatus === 'Rascunho') && (
                              <>
                                <DropdownMenuItem onClick={() => handleOpenModal(prop)} className="flex items-center gap-2">
                                  <Edit2 size={14} /> Editar Proposta
                                </DropdownMenuItem>
                                <DropdownMenuItem onClick={() => handleMarcarEnviada(prop.id)} className="flex items-center gap-2">
                            ...
                            {propStatus === 'Enviada' && (
                              <>
                                ...
                                <DropdownMenuItem onClick={() => handleReverterPendente(prop.id)} className="flex items-center gap-2">
                                  <ArrowLeft size={14} /> Voltar para Pendente
                                </DropdownMenuItem>
                              </>
                            )}

                            {/* RECUSADA / EXPIRADA / ENVIADA / PENDENTE */}
                            {(propStatus === 'Enviada' || propStatus === 'Pendente' || propStatus === 'Rascunho') && (
                              <DropdownMenuItem onClick={() => handleOpenRefuseModal(prop.id)} className="flex items-center gap-2 text-red-600 hover:text-red-700 hover:bg-red-50">
                                <X size={14} /> Recusar
                              </DropdownMenuItem>
                            )}

                            {(propStatus === 'Recusada' || propStatus === 'Expirada') && (
                              <DropdownMenuItem onClick={() => handleDuplicar(prop)} className="flex items-center gap-2">
                                <Copy size={14} /> Duplicar como Nova
                              </DropdownMenuItem>
                            )}

                            {/* APROVADA */}
                            {propStatus === 'Aprovada' && (
                              <DropdownMenuItem onClick={handleVerObraGerada} className="flex items-center gap-2 text-slate-700">
                                <Eye size={14} /> Ver Obra Gerada
                              </DropdownMenuItem>
                            )}

                            {/* ALWAYS AVAILABLE EXCEPT FOR RASCUNHO OR OTHERS IF YOU WISH */}
                            <DropdownMenuItem onClick={() => handleGeneratePDF(prop)} className="flex items-center gap-2" disabled={isGeneratingPdf === prop.id}>
                              {isGeneratingPdf === prop.id ? <Loader2 size={14} className="animate-spin" /> : <Download size={14} />}
                              {isGeneratingPdf === prop.id ? 'Gerando...' : 'Gerar PDF'}
                            </DropdownMenuItem>
          ...
          <div className="w-full max-w-md bg-white rounded-2xl shadow-2xl flex flex-col animate-in zoom-in-95 duration-200">
            <div className="flex items-center justify-between px-6 py-4 border-b border-slate-200 bg-white rounded-t-2xl shrink-0">
              <div className="flex items-center gap-2 text-red-600">
                <AlertCircle size={20} />
                <h2 className="text-lg font-bold">Recusar Proposta</h2>
              </div>
              <Button variant="ghost" className="h-8 w-8 p-0 rounded-full" onClick={() => setIsRefuseModalOpen(false)}>
                <X size={18} />
              </Button>
            </div>
            <div className="p-6 space-y-4">
              <div>
                ...
                <Select value={refuseReason} onChange={(e) => setRefuseReason(e.target.value)}>
              ...
              {refuseReason === 'Outro' && (
                <div>
                  ...
                  <Textarea
                    value={refuseObservation}
                    onChange={(e) => setRefuseObservation(e.target.value)}
            ...
            <div className="p-4 border-t border-slate-200 flex justify-end gap-3 bg-slate-50 rounded-b-2xl">
              <Button variant="outline" onClick={() => setIsRefuseModalOpen(false)}>Cancelar</Button>
              <Button
                onClick={handleConfirmRefuse}
                className="bg-red-600 hover:bg-red-700 text-white"
                disabled={refuseReason === 'Outro' && refuseObservation.trim() === ''}
              >
                Confirmar Recusa
              </Button>
            </div>
