...
interface ParcelaProposta {
  ...
  percentual?: number;
  prazo?: string;
  dias?: number; // Added to match usage in template
...
const ProposalPDF: React.FC<ProposalPDFProps> = ({ proposta, cliente, empresa }) => {
  ...
  const inicioPrevistoFormatado = formatDateSimple((proposta as any)?.inicioPrevisto || (proposta as any)?.dataPrevistaInicio || null);

  const tiposEstacaUnicos = Array.from(new Set(proposta?.servicos?.map(s => s?.tipoEstaca) || [])).filter(Boolean).join(' / ') || 'EQUIPAMENTOS';
  const totalMetros = (proposta?.servicos || []).reduce((acc, s) => acc + ((s?.quantidade || 0) * (s?.metragemPrevista || 0)), 0);
    ...
    <Document>
      <Page size="A4" style={styles.page}>
        ...
        <View style={{ marginBottom: 15 }}>
          ...
          <Text style={styles.textoBold}>ENDEREÇO: <Text style={styles.textoNormal}>{cliente?.enderecoFaturamento || 'Não informado'}</Text></Text>
          <Text style={[styles.textoBold, { marginTop: 10 }]}>PROPOSTA N. <Text style={styles.textoNormal}>{proposta?.id?.substring(0, 8).toUpperCase() || 'NOVO'}</Text></Text>
        ...
        {(proposta?.servicos || []).map((s, i) => (
          <View key={i} style={{ marginBottom: 4 }}>
            ...
            <Text style={styles.textoNormal}>
              Estacas d= {s?.diametro ? `Ø${s.diametro}` : 'N/D'}, comprimento previsto: {s?.quantidade || 0} estacas x {s?.metragemPrevista || 0}m = {formatMeters((s?.quantidade || 0) * (s?.metragemPrevista || 0))}
            </Text>
          </View>
        ))}
        <Text style={[styles.textoBold, { marginTop: 4 }]}>TOTAL = {formatMeters(totalMetros || 0)}</Text>

        {/* 3. PRAZO E INÍCIO */}
        <Text style={[styles.secaoTitulo, { marginTop: 15 }]}>PRAZO DE EXECUÇÃO e INÍCIO DA OBRA.</Text>
        <Text style={styles.textoNormal}>Gostaríamos de informar que o prazo de execução para esta referida obra será de {proposta?.prazoExecucao || '30'} dias trabalhados.</Text>
        ...
        <Text style={[styles.titulo, { fontSize: 10, marginTop: 10, textAlign: 'left' }]}>
          VALORES PARA PRESTAÇÃO DE SERVIÇO ESTACAS DO TIPO {tiposEstacaUnicos || 'FUNDAÇÕES'}
        </Text>
        ...
        <View style={{ marginTop: 8 }}>
          {(proposta?.servicos || []).map((s, i) => {
            ...
            return (
              <View key={i} style={styles.linha}>
                <Text style={[styles.textoNormal, { flex: 1 }]}>Estacas de Ø{s?.diametro || 'N/D'}</Text>
                ...
                <Text style={[styles.textoNormal, { flex: 2, textAlign: 'right' }]}>
                  {s?.quantidade || 0}x{s?.metragemPrevista || 0}m x {formatCurrency(s?.precoMetro || 0)} = {formatCurrency(rowTotal)}
                </Text>
              </View>
            );
          })}
          ...
          <View style={[styles.totalLinha, { marginTop: 8 }]}>
            ...
            <Text style={[styles.textoBold, { flex: 3, textAlign: 'right' }]}>
              {formatCurrency(proposta?.valorTotal || 0)} ({valorTotalExtenso || 'zero reais'})
            </Text>
          </View>
        ...
        <Text style={styles.textoNormal}>• Saldo restante: {proposta?.prazoSaldoDias || 'A combinar'} após entrega ou conclusão.</Text>
          ...
          <View style={{ marginTop: 8, marginBottom: 8 }}>
            ...
            {proposta?.parcelas.map((p, i) => (
              <View key={i} style={styles.tabelaLinha}>
                ...
                <Text style={[styles.textoNormal, { flex: 1, textAlign: 'center' }]}>{p?.dias || 0} dias</Text>
                <Text style={[styles.textoNormal, { flex: 1, textAlign: 'right' }]}>{formatCurrency(p?.valor || 0)}</Text>
              </View>
            ))}
          </View>
        )}
        ...
        <View style={{ marginTop: 10 }}>
          <View style={styles.checkboxLinha}>
            ...
            <Text style={styles.textoNormal}>Solicito a emissão da nota fiscal {proposta?.solicitaNF && (proposta as any)?.impostoNF ? `(acrescentar ${(proposta as any).impostoNF}% no valor)` : ''}</Text>
          </View>
        ...
        <Text style={styles.textoNormal}>2. Se atingido o nível de água, será cobrado um adicional de {formatCurrency((proposta as any)?.taxaAgua || 0)} por estaca.</Text>
        ...
        <Text style={styles.textoNormal}>4. Pela impossibilidade no andamento da obra, sendo os motivos por irresponsabilidade ou falta de gerenciamento da CONTRATANTE e que venha a ocorrer que nosso equipamento fique parado, será cobrado o valor de {formatCurrency(proposta?.horaParada || 0)} ({horaParadaExtenso || 'zero reais'}) por cada hora de perfuratriz inoperante no local com todos envolvidos.</Text>
