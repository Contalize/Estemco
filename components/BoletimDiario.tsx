  ...
  const onSubmit = async (data: BoletimFormData) => {
    ...
    try {
      ...
      await runTransaction(db, async (t) => {
        // Refs
        const bdoRef = doc(collection(db, 'boletins'));
        const dreRef = doc(db, 'dre_obras', data.obraId);
        const obraRef = doc(db, 'obras', data.obraId);

        // Gets
        const dreSnap = await t.get(dreRef);
        const obraSnap = await t.get(obraRef);

        let nDRE: Partial<DREObra>;

        if (dreSnap.exists()) {
          const curr = dreSnap.data() as DREObra;
          const oldMetros = curr.totalMetrosExecutados || 0;
          const newMetros = data.metrosExecutados;
          const totalMetrosApos = oldMetros + newMetros;

          // Média ponderada por metros (mais correta que por nº de boletins)
          const nvOverbreakPonderado = totalMetrosApos > 0
            ? ((curr.overbreakMedio || 0) * oldMetros + overbreakPct * newMetros) / totalMetrosApos
            : overbreakPct;

          nDRE = {
            totalMetrosExecutados: totalMetrosApos,
            totalDieselGasto: (curr.totalDieselGasto || 0) + custoDiesel,
            totalHorasParadas: (curr.totalHorasParadas || 0) + data.horasParada,
            totalHorasProducao: (curr.totalHorasProducao || 0) + data.horasProducao,
            totalCustoHorasParadas: (curr.totalCustoHorasParadas || 0) + custoHorasParadas,
            totalCustoMaoDeObra: (curr.totalCustoMaoDeObra || 0) + custoEquipeForm,
            totalCustoConcreto: (curr.totalCustoConcreto || 0) + custoConcreto,
            totalBoletins: (curr.totalBoletins || 0) + 1,
            overbreakMedio: nvOverbreakPonderado,
            updatedAt: Timestamp.now()
          };
          t.update(dreRef, nDRE);
        } else {
          // Create first hook if absent
          nDRE = {
            tenantId: profile.tenantId!,
            obraId: data.obraId,
          ...
          t.set(dreRef, nDRE as DREObra);
        }

        // update Obra counters
        if (obraSnap.exists()) {
          const currO = obraSnap.data() as ConstructionSite;
          t.update(obraRef, {
        ...
        t.set(bdoRef, bdoDoc as Boletim);
      });

      setToastMessage('BDO registrado e indicadores da obra atualizados!');
      setIsModalOpen(false);
      reset();
    } catch (err) {
      alert("Falha na sincronização do boletim. Verifique os dados.");
    } finally {
      setIsSaving(false);
    }
  };
